...
function parseHighwayRef(ref) {
  if (!ref) return null;
  const m = ref.match(/^(I|US|DE)-(\d+)$/);
  if (!m) return null;
  return { type: m[1], number: parseInt(m[2], 10), isEven: parseInt(m[2], 10) % 2 === 0 };
}

// When multiple routes are concurrent (e.g. I-95 running with DE-1 briefly
// near Wilmington), DelDOT's mile markers and exit numbers follow the
// Interstate designation, not whichever ref happened to sort first out of
// Overpass. Used for milepost lookups and the shield — NOT for the
// on-screen "I-95 / DE-1" text, which still shows the full concurrency,
// and NOT for camera matching, which already checks against every ref.
function primaryHighwayRef(list) {
  if (!list || !list.length) return null;
  const priority = { I: 0, US: 1, DE: 2 };
...
// genuine long Interstate-Interstate multiplex (both tied at type "I" —
// both should be treated as equally "primary") from a short accidental
// overlap with a lower-tier route. primaryHighwayRef() would arbitrarily
// tie-break two same-type refs by array order, which is exactly wrong for
// deciding "should this ref get the full search radius" — two same-type
// refs should both qualify, not just whichever sorted first.
...
function shieldCandidates(parsed) {
  ...
  if (parsed.type === 'I') return [`I-${n}.svg`, `Interstate ${n}.svg`, `I-${n} (DE).svg`];
  if (parsed.type === 'US') return [`US ${n}.svg`, `US Highway ${n}.svg`, `US ${n} (DE).svg`];
  if (parsed.type === 'DE') return [`DE-${n}.svg`, `Delaware ${n}.svg`, `DE ${n}.svg`];
  return [];
}
...
// ---------- Mile marker lookup ----------
// DelDOT's fields (ROAD/ROUTE/SIGN_FACING/LEGEND_TEXT) are unconfirmed
// against live sample data — the query endpoint's non-interactive fetch
// only returns an empty form, the same limitation hit with MD initially.
// Built defensively from the schema's field aliases: route matching tries
// both ROUTE and ROAD against normalizeHighwayName() so it works whichever
// field turns out to hold the actual route code; the milepost number is
// parsed out of LEGEND_TEXT (the sign's printed text — exact format
// unconfirmed); SIGN_FACING is mapped to a direction if it's a
// recognizable compass value. Unlike MD, DE's layer DOES appear to have a
// real per-marker direction field (SIGN_FACING) — if it turns out
// reliable, that's strictly better than MD's bearing-only fallback. Check
// the Debug panel's sampleRouteAttrs if mileposts don't show up correctly
// — it'll show exactly what's in each field so this can be corrected fast.
function parseDeMilemarkerRoute(attrs) {
  return normalizeHighwayName(attrs.ROUTE) || normalizeHighwayName(attrs.ROAD);
}
...
function deRouteMatches(attrs, parsedRef) {
  ...
  const routeRef = parseDeMilemarkerRoute(attrs);
  if (!routeRef) return false;
  const parsed = parseHighwayRef(routeRef);
  return parsed ? (parsed.type === parsedRef.type && parsed.number === parsedRef.number) : false;
}
...
async function queryNearestMileMarkers(lat, lon) {
  ...
  const params = new URLSearchParams({
    ...
    outFields: 'ROAD,ROUTE,SIGN_FACING,LEGEND_TEXT',
    outSR: '4326',          // otherwise geometry comes back in Web Mercator meters, not lat/lon
    returnGeometry: 'true', // no flat lat/lon attribute fields — need geometry.x/y instead
  });
  try {
...
async function updateMilepostAndDirection(lat, lon) {
  ...
  const withMeta = features.map(f => {
    ...
    const mp = parseDeMilemarkerMp(a.LEGEND_TEXT);
    const geom = f.geometry || {};
    const dist = (geom.y != null && geom.x != null)
      ? haversineMeters(lat, lon, geom.y, geom.x) // geometry.y = lat, geometry.x = lon (outSR=4326)
      : Infinity;
    return { mp, dist, lat: geom.y, lon: geom.x, attrs: a, direction: parseDeMilemarkerDirection(a.SIGN_FACING) };
  });
  ...
  for (const ref of tryRefs) {
    ...
    const routeCandidates = withMeta.filter(f => f.mp != null && deRouteMatches(f.attrs, parsedForRef));
  ...
  if (!matched) {
    setDebug({ milepostLookup: 'no matching-route markers in our direction of travel', sampleRouteAttrs: features.slice(0, 5).map(f => f.attributes && { road: f.attributes.ROAD, route: f.attributes.ROUTE, facing: f.attributes.SIGN_FACING, legend: f.attributes.LEGEND_TEXT }) });
    return;
  }
